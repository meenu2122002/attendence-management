...
router2.post("/teacherlogin", body('email').isEmail(), body('password').isLength({ min: 8 }), async (req, res) => {
  ...
  if (user) {
    ...
    if (bool) {
      res.redirect("http://localhost:3000/chooseclassandsubject")
    }
    else {
      console.log("please check your credentials");
      res.redirect(res.redirect("http://localhost:3000/teacherlogin"));
    }
    ...
    console.log("please check your credentials");
    res.redirect("http://localhost:3000/teacherlogin");
  }
});
